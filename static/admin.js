...
function openOpportunityModal() {
    if (!currentEditId) {
        ...
        const header = document.querySelector('#opportunityModal h3');
        if (header) header.textContent = 'Add New Opportunity';
...
document.getElementById('opportunityModal').addEventListener('click', function(e) {
    if (e.target === this) {
        ...
        async function loadOpportunities() {
            ...
            try {
                ...
                opps.forEach(opp => {
                    ...
                    let skillsArr = [];
                    try {
                        skillsArr = JSON.parse(opp.skills);
                    } catch(e) {
                        skillsArr = opp.skills.split(',');
                    }
                    if(!Array.isArray(skillsArr)) skillsArr = [opp.skills];
                    ...
                    const skillsHtml = `<div class="opportunity-skills"><div class="opportunity-skills-label">Skills You'll Gain</div><div class="skills-tags">
                        ${skillsArr.map(s => `<span class="skill-tag">${escapeHtml(s)}</span>`).join('')}
                    </div></div>`;
                    ...
                    const footerHtml = `
                        <div class="opportunity-footer">
                            ...
                            <div style="display:flex;gap:8px;">
                                <button class="view-course-btn view-btn" style="width: auto; padding: 8px 16px;">View</button>
                                <button class="view-course-btn edit-btn" style="width: auto; padding: 8px 16px; background-color:var(--qf-blue); border-color:var(--qf-blue); color:white;">Edit</button>
                                <button class="view-course-btn delete-btn" style="width: auto; padding: 8px 16px; background-color:var(--qf-red); border-color:var(--qf-red); color: white;">Delete</button>
                            </div>
                        </div>
                    `;
                    ...
                    card.querySelector('.edit-btn').addEventListener('click', () => {
                        ...
                        document.getElementById('oppSkills').value = skillsArr.join(', ');
                    ...
                    card.querySelector('.delete-btn').addEventListener('click', async () => {
                        if (confirm('Are you sure you want to delete this opportunity?')) {
                            try {
                                const delRes = await fetch('/api/opportunities/' + opp.id, { method: 'DELETE' });
                                if (delRes.ok) {
        ...
        document.getElementById('opportunityForm').addEventListener('submit', async function(e) {
            ...
            // parse skills
            const skills = JSON.stringify(skillsRaw.split(',').map(s => s.trim()).filter(Boolean));

            const oppData = {
                name, duration, start_date: startDate, description, skills, category, future_opportunities: futureOpportunities, max_applicants: maxApplicants
            };

            try {
                if (currentEditId) {
                    const res = await fetch('/api/opportunities/' + currentEditId, {
                        method: 'PUT',
...
document.getElementById('loginForm').addEventListener('submit', async function(e) {
    ...
    const remember = document.querySelector('.remember-me input') ? document.querySelector('.remember-me input').checked : false;
